...
public:
    ...
    void printInfo(const std::string& message) {
        std::cout << CYAN << "[i] " << RESET << message << std::endl;
    }

    void printSection(const std::string& message) {
        std::cout << BLUE << BOLD << "[=== " << message << " ===]" << RESET << std::endl;
    }
    ...
    void displayBanner() {
        ...
        std::cout << CYAN;
        std::cout << "           ClaudeMods Vanilla Arch Tty Grub to CachyOS Tty Grub v1.0 24-10-2025" << std::endl;
        std::cout << RESET;
        std::cout << "================================================================================" << std::endl;
        std::cout << std::endl;
    }
    ...
    void runMigration() {
        ...
        std::cout << "• Changing system themes and configurations" << RESET << std::endl;
        std::cout << std::endl;
        std::cout << RED << BOLD << "Ensure you have backups and understand the risks before proceeding!" << RESET << std::endl;
        std::cout << std::endl;
        ...
        executeCommand("sudo cp -r /home/$USER/vanillaarch-to-cachyos/install-fulltty-grub/grub /etc/default");
        printStatus("GRUB configuration copied");

        printInfo("Generating new GRUB configuration...");
        executeCommand("sudo grub-mkconfig -o /boot/grub/grub.cfg");
        printStatus("GRUB configuration updated");

        printInfo("Setting Plymouth boot animation...");
        executeCommand("sudo plymouth-set-default-theme -R cachyos-bootanimation");
        printStatus("Plymouth theme configured");

        printInfo("Configuring Fish shell...");
        executeCommand("mkdir /home/$USER/.config/fish");
        executeCommand("cp -r /home/$USER/vanillaarch-to-cachyos/install-fulltty-grub/config.fish /home/$USER/.config/fish/config.fish");
        executeCommand("cp -r /home/$USER/vanillaarch-to-cachyos/install-fulltty-grub/.zshrc /home/$USER/.zshrc");
        executeCommand("sudo chmod +X /home/$USER/.config/fish/config.fish");
        executeCommand("chsh -s $(which fish)");
        ...
        executeCommand("sudo rm -rf /home/$USER/vanillaarch-to-cachyos");
